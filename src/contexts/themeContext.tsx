...
export const ThemeContextProvider: FC<IThemeContextProviderProps> = ({ children }) => {
	...
	const [darkModeStatus, setDarkModeStatus] = useState(
		localStorage.getItem('virtuepoker_darkModeStatus')
			? localStorage.getItem('virtuepoker_darkModeStatus') === 'true'
			: process.env.REACT_APP_DARK_MODE === 'true',
	);

	useLayoutEffect(() => {
		localStorage.setItem('virtuepoker_darkModeStatus', darkModeStatus.toString());
	}, [darkModeStatus]);
	...
	const [asideStatus, setAsideStatus] = useState(
		localStorage.getItem('virtuepoker_asideStatus')
			? localStorage.getItem('virtuepoker_asideStatus') === 'true'
			: // @ts-ignore
			  deviceScreen?.width >= process.env.REACT_APP_ASIDE_MINIMIZE_BREAKPOINT_SIZE,
	);
	useLayoutEffect(() => {
		localStorage.setItem('virtuepoker_asideStatus', asideStatus?.toString());
	}, [asideStatus]);
	...
	useLayoutEffect(() => {
		...
		if (deviceScreen?.width >= process.env.REACT_APP_ASIDE_MINIMIZE_BREAKPOINT_SIZE) {
			if (localStorage.getItem('virtuepoker_asideStatus') === 'true') setAsideStatus(true);
